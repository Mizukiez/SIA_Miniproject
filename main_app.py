...
import movies_app, music_app, games_app, db
import theme
...
class MainApp:
    ...
    def show_main_menu(self):
        ...
        # Logout Button (Styled as a text link or soft button)
        ...
        apps = [
            {"name": "Music & Audio", "icon": "🎵", "cmd": lambda: self.open_app(music_app.MusicApp),
             "color": "#ecfdf5"},  # Mint hint
            {"name": "Movies & TV", "icon": "🎬", "cmd": lambda: self.open_app(movies_app.MoviesApp),
             "color": "#eff6ff"},  # Blue hint
            {"name": "Video Games", "icon": "🕹️", "cmd": lambda: self.open_app(games_app.GamesApp), "color": "#f5f3ff"}
            # Purple hint
        ]
    ...
    def create_app_card(self, parent, app, col):
        # Card Style: White background, subtle gray border
        ...
        # Hover Animation
        def on_enter(e):
            card.config(bg=app.get("color", theme.HOVER_COLOR))  # Slight color tint on hover
        ...
        # Inner content container (to center items)
        content_box = tk.Frame(card, bg=theme.CARD_COLOR)  # Inherits hover color via bind logic below
